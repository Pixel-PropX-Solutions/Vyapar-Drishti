    ...
    return (
        ...
        >
            <View style={{ width: '100%' }} >
                ...
                {productsNo && <TextTheme isPrimary={false} style={{ paddingLeft: 2, fontWeight: 600, fontSize: 12 }} >#{productsNo}</TextTheme>}
            </View>
            ...
            <ShowWhen when={inStock <= lowStockQuantity} >
                <View style={{ paddingHorizontal: 12, position: 'absolute', top: 8, right: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderWidth: 2, borderColor: 'white', boxSizing: 'border-box', height: 32, backgroundColor: !(inStock > 0 && inStock <= lowStockQuantity) ? 'rgb(250,50,50)' : 'rgb(250,150,50)' }} >

                    <Text style={{ fontSize: 12, color: 'white' }} >
                        {(inStock > 0 && inStock <= lowStockQuantity) ? 'Low Stock' : 'Out of Stock'}
                    </Text>
                    ...
                    <BackgroundThemeView style={{ position: 'absolute', borderRadius: '50%', aspectRatio: 1, padding: 2, top: 5, right: 3, transform: 'translate(50%, -50%)' }} >
                        <AnimatePingBall size={12} backgroundColor={!(inStock > 0 && inStock <= lowStockQuantity) ? 'rgb(250,50,50)' : 'rgb(250,150,50)'} />
                    </BackgroundThemeView>
                </View>
            </ShowWhen>
        </AnimateButton>
