...
export const StatsCard = ({ rgb, label, value, loading, valueSize = 14, labelSize = 10 }: { rgb: string, label: string, value: string, loading: boolean, valueSize?: number, labelSize?: number }): React.JSX.Element => (
    <View style={{ backgroundColor: `rgba(${rgb},0.2)`, flex: 1, borderRadius: 12, position: 'relative', overflow: 'hidden' }} >
        {loading ? (
            ...
            <>
                <View style={{ width: '100%', position: 'absolute', bottom: 0, left: 0, height: 4, backgroundColor: `rgba(${rgb}, .4)` }} />
