...
export function useProduct(index?: number, ...dipendency: any): ReturnType {
    ...
    useEffect(() => {
        const amount = roundToDecimal(data.rate * data.quantity);
        const tax_amount = roundToDecimal(amount * data.tax_rate / 100);
        const total_amount = roundToDecimal(amount - data.discount_amount + tax_amount);
